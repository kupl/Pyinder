...
module State (Context : Context) = struct
  ...
  and forward_expression ~resolution { Node.location; value } =
    ...
    | ComparisonOperator
        ...
        print_endline "ComparisonOperator2";
        let resolve_in_call
  ...
  let forward_statement ~resolution ~statement:{ Node.location; value } =
    ...
    | Return { Return.expression; is_implicit } ->
        (match Resolution.parent resolution with
        | Some p ->
          Format.printf "%a \n\n" Reference.pp p
        | None -> ());
...
let exit_state ~our_model ~resolution (module Context : Context) =
  ...
  let global_resolution = Resolution.global_resolution resolution in
  let { Node.value = { Define.signature = { Define.Signature.name; Define.Signature.parent; _ }; _ } as define; _ } =
    Context.define
  in
  let _ = our_model in
  let resolution = 
  ...
  if Define.is_stub define then
    ...
    *)
    ...
    (*
    (match exit with
    | Some e -> Format.printf "[ Exit Fixpoint ] \n\n%a\n\n" State.pp e
    | None -> ()
    );
    *)
    ...
    *)
    ...
    let last_state = Hashtbl.find fixpoint.preconditions Cfg.exit_index in
    (match parent with
    | Some reference ->
      (match last_state with
        | Some state ->
          ...
          | Value v -> 
            (*Format.printf "\n\n [[[ TEST ]]] \n\n %a \n\n" Resolution.pp v;*)
            OurTypeSet.ClassSummary.outer_join ~global_resolution (OurTypeSet.OurSummary.class_summary !our_model) reference (Resolution.annotation_store v);
          | Unreachable -> ()
          )
        | None -> ()
      )
...
let compute_local_annotations ~global_environment name =
  ...
  let exit_state_of_define define_node =
    ...
    exit_state ~our_model ~resolution (module Context)
  in
...
let check_define
  ...
  let errors, local_annotations, callees =
    try
      ...
      let type_errors, local_annotations, callees = exit_state ~our_model ~resolution (module Context) in
      let errors =
